        ...
        <v-list-item class="brand" link to="/">
          <v-list-item-content>
            <v-list-item-title
              ...
              style="background:transparent;line-height:1.2rem;"
            >
              <span class="primary--text"
                >V<span class="hidden-sm-and-down">uetify</span></span
              >
      ...
      <v-menu left offset-y transition="slide-y-transition" bottom>
        ...
        <v-list dense>
          <v-list-item
            router
            link
            v-for="dropdownitem in profiledropdown"
            :key="dropdownitem.id"
            :to="dropdownitem.link"
          >
            <!-- :to="{ name: 'Profile' }" -->
            <v-list-item-title>
              <v-icon class="mr-2">{{ dropdownitem.icon }}</v-icon>
    ...
    <v-navigation-drawer v-model="drawer" clipped app>
      <v-list class="nav" dense>
        <span v-for="navitem in navitems" :key="navitem.id">
          <v-subheader v-if="navitem.subtitle">{{
            $t(navitem.subtitle)
          }}</v-subheader>
              ...
              <span v-for="subitem in navitem.subitems" :key="subitem.id">
                ...
                <span v-else>
                  <v-list-item sub-group router link :to="subitem.routes">
                    <v-list-item-content>
                      <!-- child -->
                      <v-list-item-title style="margin-left: 57px;">{{
                        $t(subitem.title)
                      }}</v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
